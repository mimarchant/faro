...
<template>
  <header>
    ...
    <p class="tagline">
      ...
      <code>PageSpeed Insights API</code>. Mismos scores y Core Web Vitals que ves en PageSpeed, con
      exportación lista para pegar en una IA.
    </p>
  </header>
  ...
  <div v-if="loading" class="loading">
    <div class="scanner">
      <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
        <circle cx="32" cy="32" r="28" stroke="rgba(120,140,165,0.18)" stroke-width="4" />
        <path d="M32 4 a28 28 0 0 1 28 28" stroke="var(--accent)" stroke-width="4" stroke-linecap="round" />
      </svg>
    </div>
    <p>Auditando <span class="target">{{ url }}</span> · móvil y escritorio…</p>
  ...
  <div v-if="!loading && !generalError && !hasAny && (errors.mobile || errors.desktop)" class="error">
    <strong>No se pudo completar</strong>
    {{ errors.mobile || errors.desktop }}
  </div>

  <!-- vacío -->
  <div v-if="!loading && !hasAny && !generalError && !errors.mobile && !errors.desktop" class="empty">
  ...
  <div v-if="!loading && hasAny" class="results">
    ...
    <div v-if="!activeResult && activeError" class="error">
      <strong>{{ view === 'mobile' ? 'Móvil' : 'Escritorio' }} no disponible</strong>
      {{ activeError }}
    </div>

    <template v-if="activeResult">
      <ScoreGauges :key="view + '-gauges'" :categories="activeResult.categories" />
      ...
      <template v-if="activeResult.opportunities.length">
        <h2 class="section-title">
          Oportunidades de rendimiento <span class="tag">{{ activeResult.opportunities.length }}</span>
        </h2>
        <p class="sec-note">Toca una fila para ver los recursos concretos que la provocan.</p>
        <AuditList :key="view + '-opp'" :audits="activeResult.opportunities" delay="0.15s" />
      </template>
      ...
      <template v-if="activeResult.diagnostics.length">
        ...
        <AuditList :key="view + '-diag'" :audits="activeResult.diagnostics" delay="0.16s" />
      </template>

      <template v-if="activeResult.issues.accessibility.length">
        <h2 class="section-title">
          Accesibilidad <span class="tag">{{ activeResult.issues.accessibility.length }} por revisar</span>
        </h2>
        <AuditList :key="view + '-a11y'" :audits="activeResult.issues.accessibility" delay="0.17s" />
      </template>

      <template v-if="activeResult.issues.seo.length">
        <h2 class="section-title">
          SEO <span class="tag">{{ activeResult.issues.seo.length }} por revisar</span>
        </h2>
        <AuditList :key="view + '-seo'" :audits="activeResult.issues.seo" delay="0.18s" />
      </template>

      <template v-if="activeResult.issues['best-practices'].length">
        <h2 class="section-title">
          Buenas prácticas <span class="tag">{{ activeResult.issues['best-practices'].length }} por revisar</span>
        </h2>
        <AuditList :key="view + '-bp'" :audits="activeResult.issues['best-practices']" delay="0.19s" />
      </template>
    </template>
  ...
  <footer>
    <p>
      Faro · datos de
      <a href="https://developers.google.com/speed/docs/insights/v5/about" target="_blank" rel="noopener"
